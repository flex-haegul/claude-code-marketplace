...
### 3단계: 커밋 분할 여부 확인
diff를 분석한 후, AskUserQuestion 도구를 사용하여 커밋 분할 여부를 사용자에게 질문한다.

**질문 시 제공할 정보:**
- 변경된 파일 목록과 각 파일의 변경 요약을 간략히 보여준다.
- 분할이 유의미해 보이는 경우, 추천 분할 계획(각 커밋에 포함될 파일과 메시지 요약)을 옵션 설명에 포함한다.

**AskUserQuestion 옵션:**
- **"한 번에 커밋" (Recommended)**: 모든 변경사항을 하나의 커밋으로 생성한다.
- **"커밋 분할"**: 제안된 분할 계획에 따라 여러 커밋으로 나눈다.

**사용자가 "커밋 분할"을 선택하면:**
- 의존성/논리적 순서를 고려하여 커밋 순서를 자동 정렬한다 (예: 인프라 → 로직 → 테스트).
- 각 그룹별로 `git add <파일들>` → `git commit` 을 순차 실행한다.
